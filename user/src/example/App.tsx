...
function App() {
  ...
  const getContent = () => {
    ...
    if (count >= 5 && !booleanValue) {
      return <ExamStop />;
    }

    if ((fullScreen.status === 'on' && tabFocus.status === false) && count<5 && !booleanValue) {
      if (document.exitFullscreen) {
        document.exitFullscreen();
      }return <ExamPaused />
    }
  
     if ((fullScreen.status === 'off' || tabFocus.status === false) && count<5 && !booleanValue) {
      return <ExamPaused />
    }
  
    // If none of the above conditions match, display the Exam component
    if(fullScreen.status === 'on' && tabFocus.status === true){ 
      return <Exam />;}
  };
  ...
  return (
    <>
      ...
      { (count<5 && !booleanValue)? ( <Alerts fullScreen={fullScreen} tabFocus={tabFocus} />):("")}
